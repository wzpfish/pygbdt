...
class Tree(object):
  ...
  It hold the root node and restore all leaf nodes and non-leaf nodes in the tree.
  """
  ...
  def build(self, dataset, targets):
    """Build the tree to fit the dataset.
    """
  ...
  def _find_split_point(self, node, targets, sample_count=10):
    """For the given intermediate node, find the optimal split point,
    given the split strategy of type `criterion_type`
    """
    max_improvement = -1
